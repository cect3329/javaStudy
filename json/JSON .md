...
现在大多数网站，都是局部刷新，不刷新整个页面的情况下，实现页面更新；
...
**在 JavaScript 语言中，一切都是对象**(var 就完事)。因此，任何JavaScript 支持的类型都可以通过 JSON 来表示，例如字符串、数字、对象、数组等。看看他的要求和语法格式：
...
（**前后端分析，数据交互就变得异常重要，JSON格式的数据容易解析**）
...
# 2.Controller返回JSON数据（SpringMVC）
...
3. 我们随便编写一个User的实体类，然后我们去编写我们的测试Controller；
...
4. 这里我们需要两个新东西，一个是@ResponseBody，一个是ObjectMapper对象，我们看下具体的用法
...
<mvc:annotation-driven>
   <mvc:message-converters register-defaults="true">
       ...
       <!--这边是Jackson自带的Http消息格式转换，这边将其设置为false，这样子就会使用Spring的消息格式转换（应该是这个样子） -->
...
在类上直接使用 **@RestController** ，这样子，里面所有的方法都只会返回 json 字符串了，不用再每一个都添加@ResponseBody ！我们在前后端分离开发中，一般都使用 @RestController ，十分便捷！（指定方法返回json字符串，而不是返回给视图解析器处理）
