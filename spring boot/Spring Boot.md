...
- Spring Boot 通过整合整个Spring 的整个系列技术栈，来完成和简化 企业级开发
- Spring Boot 自动配置 Spring的相关配置，用户只需要配置少量的东西 ===>用户用少量的入口 来打开 J2ee企业级开发的大门（为了方便开发呗）

![image-20201116211820466](Spring Boot.assets/image-20201116211820466.png)

- 缺点：入门容易，精通难（要先了解Spring 底层框架的API）====>这是我的缺点
...
一个应用应该是一组小型服务（一系列小型服务的组合）；可以通过**HTTP的方式进行互通**；小型服务运行在自己的进程内

单体应用：ALL IN ONE ===>所有的东西都写在一个应用里面

![image-20201116212441320](Spring Boot.assets/image-20201116212441320.png)

微服务：每一个功能元素最终都是一个可独立替换和独立升级的软件单元；（微服务架构就是将**每个功能元素**放进一个独立的服务中）
...
![image-20201117214634599](Spring Boot.assets/image-20201117214634599.png)
...
### 2、导入spring boot相关的依赖（在Maven pom.xml文件中）
...
将这个应用打成jar包，直接使用java -jar的命令进行执行； ===> 无须像以前那样，将web应用打包成war包，然后将其部署在tomcat服务器上

![image-20201117220529499](Spring Boot.assets/image-20201117220529499.png)
...
- 父项目有什么用？ ===>以前开发一般用来做依赖管理（版本仲裁）

```xml
<parent>
    <groupId>org.springframework.boot</groupId>
    <artifactId>spring-boot-starter-parent</artifactId>   <!--Spring boot所有Starter的父项目 -->
    <version>1.5.9.RELEASE</version>
</parent>
...
spring-boot-dependencies他来真正管理Spring Boot应用里面的所有依赖版本；
...
//run()方法传入的类必须是@SpringBootApplication 注解所标注的类
```
